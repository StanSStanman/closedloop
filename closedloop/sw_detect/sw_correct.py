...
def realign_sw_epo(epo_fname, eve_fname, t_dist=.05, n_jobs=32):
    ...
    # Parellel two-steps detection
    good_eve, bad_ev_idx, maxneg_chs, sw_types = zip(*Parallel(n_jobs=n_jobs,
                                                               backend='loky')
        (delayed(two_steps_detect)
        (epo_idx, epo, eve=epochs.events, sfreq=epochs.info['sfreq'], 
         ch_names=epochs.ch_names, samp_range=samp_range, coords=coords, 
         radius=radius, t_zero=t_zero)
        for epo_idx, epo in tqdm(enumerate(epochs))))
    
    good_eve = [ge for ge in list(good_eve) if ge is not None]
...
def two_steps_detect(epo_idx, epo, eve, sfreq, ch_names, samp_range, 
                     coords, radius, t_zero):
    
    good_eve, bad_ev_idx = None, None
    maxneg_chs, sw_types = None, None
    
    samp_data = epo[:, samp_range]
    maxneg_ch = np.where(samp_data == samp_data.min())[0][0]
    
    ch_data = np.expand_dims(epo[maxneg_ch, :], 0)
    ...
    selected_data = epo[selected_indices, :]
    ...
    sw_mn = detect_sw(ch_data, sfreq=sfreq, hypno=None, 
                        ch_names=['envelope'], half_wlen=(0.125, 1.), 
    ...
    if any(tp in samp_range for tp in sw_mn['envelope']['maxnegpk']):
        ...
        sw_sn = detect_sw(_ch_data, sfreq=sfreq, hypno=None, 
                            ch_names=['envelope'], half_wlen=(0.125, 1.), 
        ...
        if any(tp in samp_range for tp in sw_sn['envelope']['maxnegpk']):
            ...
            new_eve = eve[epo_idx, :].copy()
            new_eve[0] += tp_diff
            good_eve = new_eve
            
            maxneg_chs = ch_names[maxneg_ch]
